    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className='mx-auto max-w-[1440px] px-4 lg:px-12 flex items-center justify-between py-5'>
        ...
        <div className='flex flex-1 items-center sm:justify-end gap-x-4 sm:gap-x-8'>
          ...
          {/* Gift Pool button — visible only when logged in */}
          {user && (
            <button
              onClick={() => navigate('/gift/create')}
              className='hidden sm:flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full bg-[#41334e] text-white hover:bg-[#5a4870] transition-all cursor-pointer whitespace-nowrap'
            >
              🎁 Gift Pool
            </button>
          )}
            ...
            {user ? (
                ...
                >
                  <UserButton.MenuItems>
                    ...
                    <UserButton.Action
                    label="My Gift Pools 🎁"
                    onClick={()=>navigate("/gift/my-pools")}
                    />
                  </UserButton.MenuItems>
                </UserButton>
              )
