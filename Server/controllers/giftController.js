...
// ─────────────────────────────────────────
// 8. ADMIN: GET ALL GIFT POOLS
// ─────────────────────────────────────────
export const getAllGiftPools = async (req, res) => {
    try {
        if (req.user?.role !== 'owner') return res.json({ success: false, message: "Not Authorized" })

        const pools = await GiftPool.find({ status: { $ne: 'pending_payment' } })
            .populate("products.productId")
            .sort({ createdAt: -1 })
        ...
        console.log("getAllGiftPools error:", error.message)
        res.json({ success: false, message: error.message })
    }
}

// ─────────────────────────────────────────
// 9. ADMIN: PROCESS EXPIRED POOLS
